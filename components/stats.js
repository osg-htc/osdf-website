...
function StatCard({ title, value, unit, rate, humanFunction, subtext, icon }) {
  ...
  return (
    <>
      <div class="p-4 bg-gray-800 rounded shadow-2xl flex flex-col text-white transition ease-in-out delay-75 hover:scale-105 hover:bg-gray-700">
        <div class="flex flex-row w-full">
          <div class="flex flex-col justify-center">
            <h4 class="text-lg text-gray-300 uppercase">{title}</h4>
            <h3 className='text-2xl font-bold'>{localValue ? (
              humanFunction(value + localAdd)
            ) : (
              <svg role="status" class="mr-2 w-8 h-8 text-gray-900 animate-spin fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="ml-auto self-center rounded-full bg-green-500 p-2">{icon}</div>
        </div>
        <div class="flex flex-row items-end">
          <div class="mt-2">
            <span class="flex-row flex items-end">
